...
export interface BodyOptions {
  ...
  attribution: boolean;
  /** Background color for the image render, as mermaid.ink's `bgColor` (hex without `#`, or `!name`); empty for none. */
  background: string;
}
...
export function buildBody(key: string, code: string, options: BodyOptions): string {
  ...
  const image =
    background === '' ? links.image : `${links.image}?bgColor=${encodeURIComponent(background)}`;
  const lines = [
    marker(key),
    options.type === 'image'
