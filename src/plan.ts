...
export const COMMENT_TYPES = ['link', 'image'] as const;
export type CommentType = (typeof COMMENT_TYPES)[number];

export interface BodyOptions {
  theme: string;
  /** `link`: a sentence with view and edit links. `image`: the rendered diagram, linking to the editor. */
  type: CommentType;
  /** Append the "Generated by" footer. */
  attribution: boolean;
  /** `bgColor` for the light image render (hex, with or without `#`, or `!name`); empty for none. */
  background: string;
  /** `bgColor` for the dark image render, shown to readers in dark mode; empty for none. */
 ...
 */
function picture(code: string, options: BodyOptions, links: PreviewLinks): string {
  const withBackground = (url: string, background: string) => {
