...
| `type`      | `link`                | `link`: a sentence with view and edit links. `image`: the rendered diagram, linking to the editor; readers in dark mode get the dark theme. |
| `background` | (none)               | Background for the light-mode image: a hex value or `!name`. Only with `type: image`. |
| `background-dark` | (none)          | Background for the dark-mode image. Same forms. Only with `type: image`. |
