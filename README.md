...
GitHub Action that comments a rendered preview of every changed `mermaid` code block in a pull
request's Markdown files, with a link to open it in [mermaid.live](https://mermaid.live), as a
review comment spanning the block, and keeps the comment current as the block changes.

GitHub renders mermaid blocks in the rendered file view, but not in the diff. This action puts
the rendered diagram, linked to mermaid.live, exactly where the reviewer is reading the source.
...
| `pr-number`   | the triggering PR     | Pull request number. Defaults to the pull request that triggered the run.                                                          |
| `theme`       | `default`             | Mermaid theme for the link and the light-mode image (`default`, `dark`, `forest`, `neutral`, or a custom name). Dark-mode readers always get `dark`. |
| `type`        | `image`               | `image`: the rendered diagram from mermaid.ink, linking to mermaid.live. `link`: a "View in mermaid.live" text link only.          |
| `attribution` | `true`                | Append a small "Generated by" line linking to this listing. `false` omits it.                                                      |
...
- Comments are identified by an HTML marker carrying the file path and the block's ordinal in
  ...
  line linking to this listing unless `attribution` is `false`.
