...
export type BodyShape =
  | { type: 'link'; attribution?: boolean }
  | { type: 'image'; pakoDark: string; attribution?: boolean };
...
export function body(key: string, pako: string, shape: BodyShape): string {
  ...
  const main =
    shape.type === 'image'
      ? `<a href="${edit}"><picture>` +
        `<source media="(prefers-color-scheme: dark)" srcset="https://mermaid.ink/img/pako:${shape.pakoDark}?bgColor=0d1117">` +
        `<img alt="Mermaid diagram, opens in mermaid.live" src="https://mermaid.ink/img/pako:${pako}?bgColor=ffffff">` +
        '</picture></a>'
      : `[View in mermaid.live](${edit})`;
  const footer = shape.attribution === false ? '' : `\n\n${FOOTER}`;
